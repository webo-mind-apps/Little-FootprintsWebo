...
class m_payStub extends CI_Model
{
    ...
    public function get_deduction1($data = null)
    {


        $year  = $data['year'];
        $sdate = date('Y-m-d H:i:s', strtotime($year . '-' . $data['month'] . '-01'));
        $edate =  date('Y-m-t  H:i:s', strtotime($year . '-' . $data['month'] . '-20'));
        $this->db->group_by('p.empid');
        $this->db->select('p.*');
    ...
    public function deductionYtd1($empid = null, $sdate = null, $edate = null)
    {
