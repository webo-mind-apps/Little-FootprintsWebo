...
<head>
	...
	<script src="<?php echo base_url(); ?>my_assets/assets/js/printThis.js"></script>

	<script>
		$(function() {
			var d = new Date();
			d.setFullYear(d.getFullYear() + 10);
			var date = $('.payment_date_js').datepicker({
				dateFormat: 'dd-M-yy',
				changeMonth: true,
				changeYear: true,
...
<body>
				...
				<div class="card">
					<div class="card-header header-elements-inline">
						...
						<div class="right" id="import-btn-group">
							<button class=" btn btn-small btn-success" id="print" type="button">Print</button>
							<button class=" btn btn-small btn-primary" id="import-btn">Import &nbsp;&nbsp;&nbsp;<i class="fa fa-download" aria-hidden="true"></i></button>
							<br>
							<a href="#!" style="float:right" class="text-right" id="generate-download">Download Format</a>
						</div>
					</div>
					<div class="card-body">
						<?php
							...
							<table class="table ">
								<tr class="table-active table-border-double">
										...
										<select id="companySelect" name="company">
											<?php foreach ($company as $key => $value) {
												if (isset($_GET["company"])) {
													$select =  ($_GET["company"] == $value->id) ? 'selected' : '';
												}
												$select = '';
									...
									<td width="18%">
										...
										<input type="text" value="<?= isset($_GET["sdate"]) ? $_GET["sdate"] : '' ?>" onchange="payment_date_empty()" class="payment_date_js" id="payment_date" name="pay_date_val" style="margin-left:5px;font-size:12px;" autocomplete="off">
									</td>
									<td width="18%">
										<label>PAY PERIOD END DATE</label><br>
										<input type="text" value="<?= isset($_GET["edate"]) ? $_GET["edate"] : '' ?>" onchange="payment_date_empty()" class="payment_date_js" id="pay_end_date" name="pay_end_date_val" style="margin-left:12px;font-size:12px;" autocomplete="off">
									</td>
	...
	<script>
		...
		$(document).ready(function() {
			...
			$('#payment_date , #pay_end_date, #companySelect, #payment_date_on').change(function() {
			...
			$('#print').click(function(e) {
				...
				$("#payroll_details").printThis({
					header: "<center><h4>Employee Payroll</h4></center><br/></br>"
				});
			});
		});
		...
		function payroll_show_function() {
			...
			var payment_date_on = $("#payment_date_on").val();
			var date = '';

			if (payment_date != '' && pay_end_date != '' && companySelect != '' && payment_date_on != '') {
				...
				jQuery.ajax({
					...
					success: function(response) {
						...
						if (params.has('print') && shallPrint) {
							$('#print').click();
							shallPrint = false;
						}
					}
